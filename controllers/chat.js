...
function messages() {
	...
	MAIN.refresh = function() {
		MSG_CDL.channels = MAIN.channels;
		MSG_CDL.users = MAIN.users;
		var is = true;
	...
	self.on('message', function(client, message) {
		...
		switch (message.type) {
			...
			// Real message
			case 'message':
				!client.user.blocked && MAIN.sendmessage(client, message);
				break;
		}
	});
}

MAIN.sendmessage = function(client, message) {
	if (!client.threadid || !client.threadtype)
		return;
	var self = MAIN.websocket;
	...
	if (client.threadtype === 'user') {
		...
		if (is) {
			tmp = MAIN.users.findItem('id', client.threadid);
	...
	var dbname = client.threadtype === 'channel' ? client.threadtype + client.threadid : 'user' + MAIN.merge(client.threadid, message.idto || iduser);
